...
func (c *qstatCollector) updateQstatNode(ch chan<- prometheus.Metric) {
    ...
    for _, ss := range qstat.NodeState {
        ...
        // 각 노드별 라벨 설정
        labelsValue := []string{
            ss.NodeName, ss.Mom, ss.Ntype, ss.State, ss.Jobs,
            ss.ResourcesAvailableArch, ss.ResourcesAvailableHost,
            ss.ResourcesAvailableApplications, ss.ResourcesAvailablePlatform,
            ss.ResourcesAvailableSoftware, ss.ResourcesAvailableVnodes,
            ss.Sharing,
        }

        allMetrics = append(allMetrics, nodeMetrics...)
        labelsValues = append(labelsValues, labelsValue)
    }
    ...
    for _, m := range allMetrics {
        ...
        for idx, labelsValue := range labelsValues {
            ch <- prometheus.MustNewConstMetric(
                desc,
                m.metricType,
                m.value,
